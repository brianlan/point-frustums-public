...
template <typename T, typename = std::enable_if_t<std::is_same<T, double>::value || std::is_same<T, float>::value>>
...
template <typename T> inline vec3<T> operator/(const vec3<T> &a, const T b) {
  if (b == 0.0) {
    AT_ERROR("denominator in vec3 division is 0"); // prevent divide by 0 errors.
  }
...
template <typename T> inline T norm(const vec3<T> &a) { return sqrt(dot(a, a)); }
