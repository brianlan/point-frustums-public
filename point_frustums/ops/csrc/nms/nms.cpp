...
std::tuple<std::vector<vec3<float>>, std::vector<face_verts>, std::vector<face_verts>, std::vector<float>>
CentersTrisPlanesVolumes(const at::Tensor &boxes) {
...
float IoU(const vec3<float> &b1_center, const face_verts &b1_tris, const face_verts &b1_planes, const float &b1_vol,
          const vec3<float> &b2_center, const face_verts &b2_tris, const face_verts &b2_planes, const float &b2_vol) {
...
at::Tensor NMSCpu(const at::Tensor &labels_t, const at::Tensor &scores, const at::Tensor &boxes,
                  const double iou_threshold, const double distance_threshold) {

  if (labels_t.numel() == 0) {
    return at::empty({0}, boxes.options().dtype(at::kBool));
  }
  ...
  auto labels = labels_t.data_ptr<int32_t>();

  at::Tensor scores_sorted_idx_t = std::get<1>(scores.sort(0)).to(at::kInt);
  auto scores_sorted_idx = scores_sorted_idx_t.data_ptr<int32_t>();
  ...
  // Algorithm: Iterate through a nested loop where the outer loop defines the
  // starting idx
  //  and the inner loop checks if any of the consecutive boxes remove the
  //  current one. If a match is found, set the remains entry to 0 and break the
  //  inner loop.
  for (int32_t _i = 0; _i < N; _i++) {
    // Retrieve the index that accesses the sorted scores
    auto i = scores_sorted_idx[_i];
    auto label_i = labels[i];
    for (int32_t _j = _i + 1; _j < N; _j++) {
      // Retrieve the index that accesses the sorted scores
      ...
      // If the center distance is large enough, there is no need to evaluate
      // the IoU
      if ((norm(centers[i] - centers[j]) / volumes[i]) > distance_threshold)
        continue;

      // Due to the sorted access, box j is guaranteed to have a higher score
      // than box i Evaluate IoU between boxes i and j, if the threshold is
      // surpassed, keep only box j
