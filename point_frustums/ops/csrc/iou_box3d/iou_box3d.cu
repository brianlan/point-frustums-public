...
__global__ void IoUBox3DKernel(const at::PackedTensorAccessor64<float, 3, at::RestrictPtrTraits> boxes1,
    ...
    for (size_t i = tid; i < N * M; i += stride) {
        ...
        if (box1_count > 0) {
            // The intersecting shape is a polyhedron made up of the triangular faces in box1_intersect.
...
std::tuple <at::Tensor, at::Tensor> IoUBox3DCuda(const at::Tensor &boxes1, // (N, 8, 3)
    ...
    TORCH_CHECK((boxes2.size(1) == 8) && (boxes1.size(1) == 8), "Boxes must have shape (8, 3)");
    ...
    IoUBox3DKernel<<<blocks, threads, 0, stream>>>(boxes1.packed_accessor64<float, 3, at::RestrictPtrTraits>(),
...
__global__ void IoUBox3DKernelPairwise(const at::PackedTensorAccessor64<float, 3, at::RestrictPtrTraits> boxes1,
                                       const at::PackedTensorAccessor64<float, 3, at::RestrictPtrTraits> boxes2,
...
std::tuple <at::Tensor, at::Tensor> IoUBox3DCudaPairwise(const at::Tensor &boxes1, // (N, 8, 3)
    ...
    TORCH_CHECK(boxes2.size(0) == boxes1.size(0),
                "The pairwise IoU3D requires boxes to be of the same shape (N, 8, 3)");
    TORCH_CHECK((boxes2.size(1) == 8) && (boxes1.size(1) == 8), "Boxes must have shape (N, 8, 3)");
    TORCH_CHECK(boxes2.size(2) == boxes1.size(2), "Boxes must have shape (N, 8, 3)");
    ...
    IoUBox3DKernelPairwise<<<blocks, threads, 0, stream>>>(boxes1.packed_accessor64<float, 3, at::RestrictPtrTraits>(),
                                                           boxes2.packed_accessor64<float, 3, at::RestrictPtrTraits>(),
