...
const int _PLANES[6][4] = {{0, 1, 2, 3},
                           {3, 2, 6, 7},
                           {0, 1, 5, 4},
                           {0, 3, 7, 4},
                           {1, 5, 6, 2},
                           {4, 5, 6, 7},};
const int _TRIS[12][3] = {{0, 1, 2},
                          {0, 3, 2},
                          {4, 5, 6},
                          {4, 6, 7},
                          {1, 5, 6},
...
inline vec3<float> ExtractVertsPlane(const Box &box, const int plane_idx, const int vert_idx) {
...
//
template<typename Box>
inline vec3<float> ExtractVertsTri(const Box &box, const int tri_idx, const int vert_idx) {
...
//
inline vec3<float>
PlaneEdgeIntersection(const std::vector <vec3<float>> &plane, const vec3<float> &normal, const vec3<float> &p0,
                      const vec3<float> &p1) {
    // The center of the plane
    const vec3<float> plane_ctr = PlaneCenter(plane);
...
//
inline std::tuple <vec3<float>, vec3<float>>
ArgMaxVerts(const std::vector <vec3<float>> &verts1, const std::vector <vec3<float>> &verts2) {
...
//
inline face_verts
ClipTriByPlaneOneOut(const std::vector <vec3<float>> &plane, const vec3<float> &normal, const vec3<float> &vout,
                     const vec3<float> &vin1, const vec3<float> &vin2) {
    ...
    const face_verts face_verts = {{vin1, pint1, pint2},
                                   {vin1, pint2, vin2}};
    return face_verts;
}
...
//
inline face_verts
ClipTriByPlaneTwoOut(const std::vector <vec3<float>> &plane, const vec3<float> &normal, const vec3<float> &vout1,
                     const vec3<float> &vout2, const vec3<float> &vin) {
